...
<style>
  ...
  body { background-color: var(--bg-color); color: var(--text-primary); font-family: var(--font-sans); line-height: 1.7; margin: 0; }
  ...
  .nav-links a { color: var(--text-muted); margin-left: 1.5rem; font-size: 0.9rem; }
  ...
  .article-title { font-size: 2.4rem; font-weight: 800; line-height: 1.2; margin: 0 0 1rem; color: #fff; }
  .tag-badge { display: inline-block; font-family: var(--font-mono); font-size: 0.75rem; background: #1c1c22; color: var(--text-muted); padding: 0.25rem 0.6rem; border-radius: 6px; margin-right: 0.4rem; border: 1px solid var(--card-border); }
  .article-body { font-size: 1.05rem; }
  .article-body h1, .article-body h2, .article-body h3 { color: #fff; margin-top: 2.5rem; margin-bottom: 1rem; }
  ...
  .back-link { display: inline-flex; align-items: center; color: var(--text-muted); font-size: 0.9rem; margin-bottom: 2rem; }
...
async function main() {
  ...
  for (const page of response.results) {
    ...
    const title = props.Title?.title[0]?.plain_text || 'Без названия';
    const slug = props.Slug?.rich_text[0]?.plain_text || page.id;
    const date = props.Date?.date?.start || page.created_time.split('T')[0];
    const tags = props.Tags?.multi_select?.map((t) => t.name) || props.Tag?.multi_select?.map((t) => t.name) || [];
...
<head>
  ...
  <style>
    ...
    .blog-card { background: var(--card-bg); border: 1px solid var(--card-border); border-radius: 12px; padding: 1.75rem; display: block; transition: transform 0.2s; }
